...
from celldiff.logging import logger, metric_logger
from celldiff.pipeline.accelerator.accelerator import (
    GroupedBatchIter, 
    Accelerator,
    DdpAccelerator,
...
from celldiff.pipeline.accelerator.model import Model
    ...
    def finetune_from_checkpoint(self):
        if self.finetune_from_checkpoint_dir is not None:
            self._load_checkpoint(
                self.finetune_from_checkpoint_dir, model_states_only=False
            )
        else:
