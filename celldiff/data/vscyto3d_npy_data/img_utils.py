...
class RandomCropResize:
    def __init__(self, spatial_size: Tuple[int, int], rescale_range: Tuple[float, float]):
        """
        Args:
            spatial_size: (out_h, out_w), final output size after resize
            rescale_range: uniform range multiplier for cropping region
        """
        self.spatial_size = spatial_size
        self.rescale_range = rescale_range
    ...
    def __call__(self, img: torch.Tensor):
        ...
        Returns:
            (..., out_h, out_w) float tensor
        """
        *leading_dims, h, w = img.shape
        scale_factor = random.uniform(*self.rescale_range)
        crop_h = int(self.spatial_size[0] * scale_factor)
        crop_w = int(self.spatial_size[1] * scale_factor)
        ...
        if pad_top > 0 or pad_bottom > 0 or pad_left > 0 or pad_right > 0:
            ...
            h, w = img.shape[-2:]  # update size

        # --- apply cropping if needed ---
        if crop_h <= h:
            top = random.randint(0, h - crop_h)
            img = img[..., top:top + crop_h, :]
        if crop_w <= w:
            left = random.randint(0, w - crop_w)
            img = img[..., :, left:left + crop_w]
